...
void first_come_first_serve_with_priority(int item[][4],int n)
{
...
void round_robin(int item[][4],int time_quantum,int num)   //applying round robin for whole system
{
	...
	jump:	for(int i=0;i < num;i++)
		{ int print =0;
			...
			if(item[i][2]<= time_quantum&& item[i][2]>0)    // if burst time is less than the time quantum only run for left time
			{
				...
				item[i][2] = 0;
				check++;
				time_elapsed += print;                // as burst time is zero the process will no longer execute so increase check to keep track
...
int main()
{
	...
	round_robin(item,time_quantum,n);
	cout<<"\nend"<<endl;
	}
